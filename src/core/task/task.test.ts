...
import { sortTasks, filterUniqueTasks, createTask } from './task.js'
...
describe('Task Utilities', () => {
  describe('createTask', () => {
    ...
    it('should create a task with all properties', () => {
      const task = createTask('build', 'Build the project', '/home/user/project')
      expect(task).toEqual({
        name: 'build',
        description: 'Build the project',
  ...
  describe('sortTasks', () => {
    it('should sort tasks alphabetically by name', () => {
      ...
      expect(sorted.map((t) => t.name)).toEqual(['build', 'dev', 'lint', 'test'])
    })
