...
import { WorkspaceProject, PackageJson } from '../../types/index.js'
...
async function checkIsWorkspace(
  ...
  if (packageManager === 'npm' || packageManager === 'yarn') {
    ...
    const packageJsonPath = join(dir, 'package.json')
    try {
      const packageJson = await parseJSON<PackageJson & { workspaces?: string[] | { packages?: string[] } }>(
        packageJsonPath
      )
...
async function listWorkspaceProjects(
  ...
  try {
    ...
    Logger.debug(`Failed to list projects using ${packageManager} command: ${(error as Error).message}, falling back to manual parsing`)
  }
...
async function listProjectsUsingCommand(
  ...
  try {
    if (packageManager === 'npm') {
      ...
      const { stdout } = await execAsync('pnpm list -r --depth -1 --json', { cwd: dir })
      const result = JSON.parse(stdout)
      ...
      if (!hasRoot) {
        const rootPackageJson = await parseJSON<PackageJson>(join(dir, 'package.json'))
      ...
      const { stdout } = await execAsync('yarn workspaces list --json', { cwd: dir })
    ...
    Logger.debug(`Error executing ${packageManager} command: ${(error as Error).message}`)
    throw error
  }
      ...
      if (entry.isDirectory()) {
        ...
        if (await fileExists(packageJsonPath)) {
          try {
            ...
            Logger.debug(`Failed to parse ${packageJsonPath}: ${(error as Error).message}`)
          }
        }
      }
    }
